...
export function SignUpPage() {
  ...
  return (
    <AuthCard
      ...
      subtitle="5 minutes. Aucune carte requise."
      topSlot={googleButton}
      bottomSlot={
        <>
          Déjà inscrit ?{' '}
    ...
    >
      {serverError && (
        <Alert variant="destructive" appearance="light" close onClose={() => setServerError('')}>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <FormItem className="flex items-start gap-2 space-y-0">
                ...
                <div className="space-y-1">
                  <FormLabel className="text-sm font-normal cursor-pointer">
                    J'accepte les{' '}
                    <Link to="/legal/terms" className="text-[var(--accent-600)] hover:underline">
                      conditions d'utilisation
                    </Link>{' '}
                    et la{' '}
                    <Link to="/legal/privacy" className="text-[var(--accent-600)] hover:underline">
          ...
          <Button type="submit" className="w-full" disabled={isSubmitting}>
            {isSubmitting ? 'Inscription...' : 'Commencer'}
          </Button>
        </form>
      </Form>
    </AuthCard>
